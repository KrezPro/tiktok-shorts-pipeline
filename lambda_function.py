...
def lambda_handler(event, context):
    ...
    url = s3.generate_presigned_url(
        "get_object", Params={"Bucket":bucket,"Key":obj["Key"]}, ExpiresIn=3600
    )
    # OpenAI
    oa = requests.post(
        ...
        json={
            "model":"gpt-4",
            "messages":[{"role":"user","content":f"Short caption, bg  {url}"}],
    ...
    pr = requests.post(
        ...
        json={"brand_id":predis_brand,"text":caption,
              "media_type":"video","video_duration":"short","media_urls":[url]}
    )
